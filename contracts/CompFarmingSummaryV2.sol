...
contract CompFarmingSummaryV2 is CompFarmingSummaryStorageV1, CompFarmingSummaryInterface, Exponential, Logging, CompFarmingSummaryErrorReporter{
    ...
    function compProfile(address account) external returns(CompProfile memory _compProfile){
        ...
        _compProfile.balance = ERC20(getCompAddress()).balanceOf(account);

        CompoundLens compoundLens = CompoundLens(getCompoundLensAddress());
        _compProfile.yetToClaimed = compoundLens.getCompBalanceMetadataExt(Comp(getCompAddress()), ComptrollerLensInterface(getUnitrollerAddress()), account).allocated;
    ...
    function _become(CompFarmingSummaryProxy proxy) public {
        ...
        uint result = proxy._acceptImplementation();

        require(result == 0, uint2str(result));
    }
